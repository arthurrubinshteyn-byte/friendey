        ...
        <div className="flex-1 overflow-x-auto px-6 py-6">
          <div className="grid grid-cols-7 gap-3 min-w-[900px]">
            {weekDays.map((date, i) => {
              ...
              return (
                <div key={dateStr} className={`flex flex-col rounded-2xl border p-3 min-h-[500px] ${isToday ? 'border-gray-800 bg-white shadow-sm' : 'border-gray-100 bg-white'}`}>
                  <div className="mb-3">
                    <div className="flex items-center justify-between">
                      <span className={`text-xs font-semibold uppercase tracking-widest ${isToday ? 'text-gray-800' : 'text-gray-300'}`}>{DAYS[i]}</span>
                      {dayTasks.length > 0 && <span className="text-xs text-gray-300">{done}/{dayTasks.length}</span>}
                    </div>
                    <span className={`text-2xl font-semibold ${isToday ? 'text-gray-800' : 'text-gray-300'}`}>{date.getDate()}</span>
                    {isToday && <div className="w-4 h-0.5 bg-gray-800 rounded-full mt-1"/>}
                  </div>
                  <div className="flex-1 space-y-1.5 overflow-y-auto">
                    {dayTasks.map(task => (
                      <div key={task.id} className="flex items-start gap-2 group p-1.5 rounded-lg hover:bg-gray-50 transition">
                        <button onClick={() => toggleTask(task)} className={`mt-0.5 w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 transition ${task.is_complete ? 'bg-gray-800 border-gray-800' : 'border-gray-300 hover:border-gray-500'}`}>
                          {task.is_complete && (
                        ...
                        <span className={`flex-1 text-xs leading-relaxed ${task.is_complete ? 'line-through text-gray-300' : 'text-gray-600'}`}>{task.title}</span>
                        <button onClick={() => deleteTask(task.id)} className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-400 transition text-sm leading-none mt-0.5">×</button>
                      </div>
                    ))}
                  </div>
                  <div className="mt-2 pt-2 border-t border-gray-50">
                    <input type="text" placeholder="+ Add task..." value={newTask[dateStr] ?? ''} onChange={e => setNewTask(prev => ({ ...prev, [dateStr]: e.target.value }))} onKeyDown={e => e.key === 'Enter' && addTask(dateStr)} className="w-full text-xs text-gray-500 placeholder-gray-300 bg-transparent outline-none py-1"/>
                  </div>
                </div>
              )
            })}
